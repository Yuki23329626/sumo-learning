...
int main (int argc, char *argv[])
{
  ...
  // interface 0 is localhost, 1 is the p2p device
    ...
    // First UE
	 ...
	 OnOffHelper clientHelper ("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), dlPort));
	...
	//  serverApps.Add (ulPacketSinkHelper.Install (remoteHost)); // install app for sever
	 // serverApps.Add (packetSinkHelper.Install (ueNodes.Get(u))); // server other port for other app
	 ...
	 // dlClient.SetAttribute ("MaxPackets", UintegerValue(1000000));
	  ...
	  if (u+1 < ueNodes.GetN ()){
		...
		{
		  clientApps.Add (client.Install (ueNodes.Get(1)));
		}
	 }
